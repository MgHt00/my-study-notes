#### Any function that can cmbined tow values (such as tow objects) into one value of the same type can be used as a reduction function (with array methods `reduce()` and `reduceRight()`.
