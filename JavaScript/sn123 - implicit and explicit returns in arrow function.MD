[G]* **Concise Form:** When an arrow function has only one expression, you can omit the curly braces `{}` and the `return` keyword. The expression itself is implicitly returned.
